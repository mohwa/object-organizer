...
export function create(v, p) {
  return Object.create(toPlainObject(v), p);
}
...
export function deepHas(v, k) {
  const _has = vv => has(vv, k);
  return deepTruly(v, { fAtObject: _has });
}
...
export function deepHasInstanceOf(v, o) {
  const _hasInstanceOf = vv => hasInstanceOf(vv, o);
